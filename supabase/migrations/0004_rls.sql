...
-- No rows in membership_outlet means "all outlets in the organisation",
-- preserving the draft's NULL-scope semantics.
...
as $$
  select exists (
    ...
    where m.organisation_id = target_org
      ...
      and m.active
      and (
        not exists (select 1 from public.membership_outlet mo where mo.membership_id = m.id)
        or exists (
          select 1 from public.membership_outlet mo
          where mo.membership_id = m.id and mo.outlet_id = target_outlet
        )
      )
  );
$$;
...
create policy outlet_read on outlet
  ...
  using ((has_org_access(organisation_id) and has_outlet_access(organisation_id, id))
         or has_staff_access(organisation_id));

create policy outlet_admin_write on outlet
...
create policy membership_read on membership
  for select to authenticated
  using (user_id = auth.uid() or has_org_role(organisation_id, array['admin']::app_role[]));
...
--   7. a row combining org A's organisation_id with org B's outlet_id is
