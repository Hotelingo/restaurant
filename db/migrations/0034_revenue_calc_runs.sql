-- 0034 · Revenue calculation persistence and queue integration
-- Slice 6 / S6-3. Extends calc-run inputs for T1B/T7 plus the actual T1
-- accounting anchor, registers stable RV/CT calc IDs, and enables Revenue
-- calculation requests only when revenue_inputs readiness is reconciled.
...
begin
  ...
  if new.input_role='actual' then
    ...
    select exists(
      select 1 from public.financial_fact f where f.batch_id=v_batch.id
    ...
    select exists(
      select 1 from public.revenue_activity_fact f where f.batch_id=v_batch.id
    ) into v_has_canonical;
    ...
    select exists(
      select 1 from public.channel_source_fact f where f.batch_id=v_batch.id
    ) into v_has_canonical;
...
values
  ('RV.ACTIVITY_UNITS','v1','RV','Activity units','activity_units','actual_units',true),
  ('RV.AVG_SPEND','v1','RV','Average spend','currency_per_activity_unit','revenue/activity_units',true),
  ('RV.REVENUE','v1','RV','Revenue','currency','actual_revenue',true),
  ('RV.VOLUME_EFFECT','v1','RV','Volume effect','currency','(Ua-Uc)*Sc',true),
  ('RV.SPEND_EFFECT','v1','RV','Spend effect','currency','Ua*(Sa-Sc)',true),
  ('RV.TOTAL_VARIANCE','v1','RV','Total revenue variance','currency','Ra-Rc',true),
  ('CT.CONTRIBUTION','v1','CT','Contribution','currency','net_sales-channel-product-labour-other_direct',true),
  ('CT.CONTRIBUTION_PER_ACTIVITY_UNIT','v1','CT','Contribution per activity unit','currency_per_activity_unit','contribution/activity_units',true),
  ('CT.CONTRIBUTION_MARGIN_PCT','v1','CT','Contribution margin percent','ratio','contribution/net_sales',true)
...
begin
  ...
  if p_queue_calc then
    if not exists(
      ...
      raise exception 'Revenue calculation requires reconciled T1B, T7 and T1 inputs'
        using errcode='check_violation';
    end if;
    ...
    select
      ...
      v_batch.id,'revenue_inputs_ready','pending'
    where not exists(
...
create or replace function request_revenue_calculation(
  p_outlet_id uuid,
  p_period_id uuid,
  p_idempotency_key text,
  p_correlation_id text default null
...
declare
  ...
  v_existing jsonb;
  v_operation text := 'revenue.calculate:'||p_outlet_id::text||':'||p_period_id::text;
  v_request_id uuid;
begin
  ...
  if v_source_batch_id is null then
    raise exception 'committed T7 source/channel batch is required'
      using errcode='check_violation';
  end if;
  ...
  values(
    ...
    'revenue_explicit_rerun','pending'
  )
  ...
  values(
    ...
    'REVENUE_CALCULATION_REQUESTED','calculation_request_queue',
...
revoke all on function request_revenue_calculation(uuid,uuid,text,text) from public;
grant execute on function request_revenue_calculation(uuid,uuid,text,text)
  to restaurant_app;
