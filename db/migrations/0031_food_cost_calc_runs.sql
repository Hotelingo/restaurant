-- 0031 · Food Cost calculation persistence and queue integration
-- Slice 5 / S5-3. Extends the immutable calc-run spine to accept pinned T2/T3/T4A
-- inputs, seeds stable FC calc definitions, and allows an explicit Food Cost
-- calculation request only after all three canonical input families are ready.
...
begin
  ...
  if new.input_role='actual' then
    ...
    if new.scenario<>'actual'
       or v_batch.template_code<>'T2'
...
values
  ...
  ('FC.ACTUAL_COST_PCT','v1','FC','Actual cost percent','ratio','actual_consumption/product_revenue',true),
  ('FC.BUDGET_BENCHMARK','v1','FC','Budget benchmark','currency','product_revenue*approved_benchmark_pct',true),
  ('FC.BUDGET_GAP','v1','FC','Budget gap','currency','actual_consumption-budget_benchmark',true),
  ('FC.EXPECTED_USAGE','v1','FC','Expected usage','currency','sum(item_units*approved_item_cost)',true),
  ('FC.EXPECTED_COST_PCT','v1','FC','Expected cost percent','ratio','expected_usage/product_revenue',true),
  ('FC.MENU_MIX_EFFECT','v1','FC','Menu mix effect','currency','expected_usage-budget_benchmark',true),
  ('FC.ACTUAL_VS_EXPECTED','v1','FC','Actual versus expected','currency','actual_consumption-expected_usage',true),
  ('FC.SUPPORTED_DRIVER_TOTAL','v1','FC','Supported driver total','currency','sum(validated_driver_impacts)',true),
  ('FC.RESIDUAL','v1','FC','Food Cost residual','currency','actual_vs_expected-supported_driver_total',true),
  ('FC.DECISION_PATH','v1','FC','Food Cost decision path','decision_path','fc_decision_path_v1',true)
on conflict(calc_id,definition_version) do nothing;


-- Four-argument overload: canonical commit plus explicit queue request. The
-- existing three-argument function remains backward compatible.
create or replace function commit_food_cost_import_batch(
  p_batch_id uuid,
  p_idempotency_key text,
  p_correlation_id text,
  p_queue_calc boolean
)
returns table (
...
begin
  ...
  from public.commit_food_cost_import_batch(
    p_batch_id,p_idempotency_key,p_correlation_id
  );
...
revoke all on function commit_food_cost_import_batch(uuid,text,text,boolean)
  from public;
grant execute on function commit_food_cost_import_batch(uuid,text,text,boolean)
  to restaurant_app;
...
declare
  ...
  v_existing jsonb;
  v_operation text := 'food_cost.calculate:'||p_outlet_id::text||':'||p_period_id::text;
  v_request_id uuid;
begin
  ...
  values(
    ...
    'food_cost_explicit_rerun','pending'
  )
  ...
  values(
    ...
    'FOOD_COST_CALCULATION_REQUESTED','calculation_request_queue',
...
revoke all on function request_food_cost_calculation(uuid,uuid,text,text)
  from public;
