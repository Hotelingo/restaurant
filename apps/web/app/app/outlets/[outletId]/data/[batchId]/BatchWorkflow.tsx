...
import { apiFetch, apiMutate, idempotencyKey } from "@/lib/api";
import type {
...
export default function BatchWorkflow({ batchId }: { batchId: string }) {
  ...
  async function read() {
    const r = await step.run(() => apiFetch<ImportParseResponse>(`/imports/${batchId}/parse`, {
      method: "POST", body: JSON.stringify({ period_id: targetPeriod, scenario }),
    }));
    if (r) { setNotice(r.profile_match_message); status.reload(); }
  }
        ...
        {batch.status === "uploaded" || batch.status === "parsing" ? (
          <Card title="Read the file">
            <div className="stack">
              ...
              <div className="actions-bar"><Button variant="primary" loading={step.busy} onClick={() => void read()}>Read file</Button></div>
            </div>
          </Card>
        ) : null}
